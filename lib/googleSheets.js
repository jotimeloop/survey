...
function getSheetsClient() {
  ...
  if (!email || !privateKeyRaw || !sheetId || sheetId.trim() === '' || sheetId === 'YOUR_SHEET_ID') {
    return null;
  }
...
export async function saveFamilyToSheet(data) {
  ...
  if (!client) {
    throw new Error(
      'Google Sheets configuration is missing or incomplete. Please ensure GOOGLE_SHEET_ID, GOOGLE_SERVICE_ACCOUNT_EMAIL, and GOOGLE_PRIVATE_KEY are set in your environment variables (.env.local or Vercel dashboard).'
    );
  }
  ...
  try {
    ...
    if (err.message && (err.message.includes('404') || err.message.includes('not found') || err.message.includes('Requested entity was not found'))) {
      throw new Error(`Google Sheet not found. Please verify GOOGLE_SHEET_ID ("${sheetId}") is a valid Google Sheet ID.`);
    }
    if (err.message && (err.message.includes('403') || err.message.includes('permission') || err.message.includes('Permission denied'))) {
      throw new Error(`Permission denied. Please open your Google Sheet, click Share, and grant Editor access to your service account email (${process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL || 'your service account email'}).`);
    }
    if (err.message && (err.message.includes('Spreadsheet') || err.message.includes('configuration') || err.message.includes('GOOGLE_SHEET_ID'))) {
      throw err;
    }
...
async function ensureHeadersExist(sheets, sheetId) {
  if (!sheetId || sheetId.trim() === '' || sheetId === 'YOUR_SHEET_ID') {
    throw new Error('Spreadsheet ID is not defined. Please set GOOGLE_SHEET_ID in your .env.local file.');
  }
  const spreadsheet = await sheets.spreadsheets.get({ spreadsheetId: sheetId });
  const sheetNames = (spreadsheet.data && spreadsheet.data.sheets || []).map(
